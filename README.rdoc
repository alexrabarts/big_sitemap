...
2. public/sitemaps/sitemap_product.xml.gz
3. public/sitemaps/sitemap_post.xml.gz

If your sitemaps grow beyond 50,000 URLs (this limit can be overridden with the <code>:max_per_sitemap</code> option), the sitemap files will be partitioned into multiple files (<code>sitemap_product_1.xml.gz</code>, <code>sitemap_product_2.xml.gz</code>, &#133;).
...
You can chain methods together.  You could even get away with as little code as:
  ...
  BigSitemap.new(:base_url => 'http://example.com').add(Post).generate
