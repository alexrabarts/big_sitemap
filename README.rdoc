...
BigSitemap is best run periodically through a Rake/Thor task.
  ...
  # Add a model
  sitemap.add(:model => Post, :path => 'articles')

  # Add another model with a change frequency and priority
  sitemap.add(
    :model            => Product,
    :path             => 'products',
...
2. public/sitemaps/sitemap_post.xml.gz

If your sitemaps grow beyond 50,000 URLs (this limit can be overridden with the <code>:max_per_sitemap</code> option), the sitemap files will be partitioned into multiple files (<code>sitemap_post_1.xml.gz</code>, <code>sitemap_post_2.xml.gz</code>, &#133;).
